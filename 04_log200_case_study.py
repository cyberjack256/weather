...
def generate_log_lines(weather_data, sun_and_moon_info, encounter_id, alias, config):
    ...
    for time, row in weather_data.iterrows():
        log_entry = {
            ...
            "sun": {
                "sunrise": sun_and_moon_info[time.date()]["sun_info"]["sunrise"],
                "noon": sun_and_moon_info[time.date()]["sun_info"]["noon"],
                "dusk": sun_and_moon_info[time.date()]["sun_info"]["dusk"],
                "sunset": sun_and_moon_info[time.date()]["sun_info"]["sunset"],
                "dawn": sun_and_moon_info[time.date()]["sun_info"]["dawn"]
            }
        }
...
def main():
    try:
        ...
        # Fetch weather data
        weather_data = fetch_weather_data(latitude, longitude, date_start, date_end)

        # Fetch sun and moon data for each date in the range
        sun_and_moon_info = {}
